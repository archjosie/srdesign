...
void snapshotWrite(int argc, char** argv, double k, double w0, double zR, unsigned int m, unsigned int n);
...
double distance1(double x, double y) { //Distance formula
	...
	for (int l = 0; l < z.size(); ++l) {  //Weird choice, but this streamlines the program
		...
		for (int i = 0; i < x.size(); ++i) {
			for (int j = 0; j < y.size(); ++j) {
				...
				double r = distance1(x.at(i), y.at(j));
	...
	for (int i = 0; i < x.size(); ++i) {
		for (int j = 0; j < y.size(); ++j) {
			for (int k = 0; k < z.size(); ++k) {
				...
				//*inhales*
				fout << x.at(i) << '\t' << y.at(j) << '\t' << z.at(k) << '\t' << ReEField.at(i).at(j).at(k) << '\t' << ImEField.at(i).at(j).at(k) << '\t' << distance1(ReEField.at(i).at(j).at(k), ImEField.at(i).at(j).at(k)) << endl;
			}
		}
	}
...
void writeData2(vector<vector<vector<double> > > ReEField, vector<vector<vector<double> > > ImEField, ofstream &fout, vector<double> x, vector<double> y) { //For case of one z-value
	...
	for (int i = 0; i < x.size(); ++i) {
		for (int j = 0; j < y.size(); ++j) {
				//*inhales*
				fout << x.at(i) << '\t' << y.at(j) << '\t' <<  distance1(ReEField.at(i).at(j).at(0), ImEField.at(i).at(j).at(0)) << endl;
		}
	}
