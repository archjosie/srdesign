...
complex<double> rTM(double n, double theta, vector<double> kvec, double WIDTH){
	theta *= PI / 180;
    vector<double> nvec;
...
complex<double> rTE(double n, double theta, vector<double> kvec, double WIDTH){
	theta *= PI / 180;
    vector<double> nvec;
    nvec.push_back(-sin(theta));
    nvec.push_back(0);
...
vector<complex<double> > eRBase (vector<complex<double> > f, double theta, vector<double>  REkVecs, double WIDTH) {
    ...
    complex<double> refTM = rTM(NVAL, theta, REkVecs, WIDTH);
    complex<double> refTE = rTE(NVAL, theta, REkVecs, WIDTH);
