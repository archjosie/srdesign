...
int main(int argc, char** argv){
	...
	fftw_complex *in, *out, *inx, *outx, *iny, *outy, *inz, *outz, *intest, *outtest;
	...
	intest = (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * FourData.size() * FourData.at(0).size());
	outtest = (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * FourData.size() * FourData.at(0).size());
    ...
    for (int i = 0; i < ERTab.size(); i++) {
        for (int j = 0; j < ERTab.at(0).size(); j++){ 
			...
			intest[k][0] = real(FourData.at(i).at(j).at(0));
			intest[k][1] = imag(FourData.at(i).at(j).at(0));

			cout << "<" << chop(intest[k][0]) << ", " << chop(intest[k][1]) << ">" << endl;
			k++;
        }    
    }
	...
	fftw_plan htest = fftw_plan_dft_2d(FourData.size(), FourData.at(0).size(), intest, outtest, FFTW_BACKWARD, FFTW_ESTIMATE);
	fftw_execute(htest);
	...
	for (int i = 0; i < ERTab.size(); i++) {
		for (int j = 0; j < ERTab.at(0).size(); j++) {
				...
				cout << "<" << outtest[k][0]/FourData.size() << ", " << outtest[k][1]/FourData.size() << ">" << endl;
				k++;
