...
int main(int argc, char** argv){
	...
	fftw_complex *in, *out, *in2, *out2, *inx, *outx, *iny, *outy, *inz, *outz;

	fftw_plan g = fftw_plan_dft_2d(beam1.getRealE().size(), beam1.getRealE().at(0).size(), in, out, FFTW_FORWARD, FFTW_ESTIMATE);
	fftw_plan f = fftw_plan_dft_2d(beam1.getRealE().size(), beam1.getRealE().at(0).size(), in2, out2, FFTW_BACKWARD, FFTW_ESTIMATE);
	...
	in2 = (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * beam1.getRealE().size() * beam1.getRealE().at(0).size());
	out2 = (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * beam1.getRealE().size() * beam1.getRealE().at(0).size());
	...
	for (int i = 0; i < beam1.getRealE().size(); i++) {
		for (int j = 0; j < beam1.getRealE().at(0).size(); j++) {
			complex<double> fourEnt(out[k][0] / 121, out[k][1] / 121);
			...
			in2[k][0] = out[k][0] / (double) 121;
			in2[k][1] = out[k][1] / (double) 121;
			k++;
		}
	}	
	...
	for (int i = 0; i < beam1.getRealE().size(); i++) {
		for (int j = 0; j < beam1.getRealE().at(0).size(); j++) {
			complex<double> fourEnt(out2[k][0], out2[k][1]);
			cout << fourEnt << endl;
			k++;
		}
	}
